#                                                             
#                                dddddddd                     
#               jjjj             d::::::d   ffffffffffffffff  
#              j::::j            d::::::d  f::::::::::::::::f 
#               jjjj             d::::::d f::::::::::::::::::f
#                                d:::::d  f::::::fffffff:::::f
#             jjjjjjj    ddddddddd:::::d  f:::::f       ffffff
#             j:::::j  dd::::::::::::::d  f:::::f             
#              j::::j d::::::::::::::::d f:::::::ffffff       
#              j::::jd:::::::ddddd:::::d f::::::::::::f       
#              j::::jd::::::d    d:::::d f::::::::::::f       
...
#         jjjjjj                                              
#
# Welcome to my user configuration, if you're looking for inspiration for your own config you've come to the right place!
# By default, NixOS really only focuses on managing your *system*, if we want to manage users we have to use home-manager.
# See here: https://github.com/rycee/home-manager
# Linux home directories are messy places, and the problem of clutter only gets worse the more software that's installed and the longer it's used.
# We can work around this somewhat by declaring and managing as much as we can, but I doubt it'll ever be perfect.
# My personal goal is to have my home managed well enough that I could delete it tomorrow and home-manager would be able to regenerate everything important.
# I'm a long way from reaching this goal.
#
...
{ config, pkgs, lib, ... }:
let
  asfSource = pkgs.fetchzip { 
    url = "https://github.com/JustArchiNET/ArchiSteamFarm/releases/download/4.2.1.1/ASF-linux-x64.zip";
    sha256 = "0bg8y3lsjhqmi0bwk6m4p1xfz495z7n7c4lnzmp80jr5f96iqszj";
    stripRoot = false;
  };
  asfPluginSource = pkgs.fetchzip { 
    url = "https://github.com/Ryzhehvost/ASF-Achievement-Manager/releases/download/0.1.2.0/ASF-Achievement-Manager.zip";
    sha256 = "1g5rsikqbb080vm9wkhricrd2bv6j71m5rj9wh5pmy7qlw45vhgf";
    stripRoot = false;
  };
  flathubApps = [
    # Here's some examples of apps I prefer to use flatpak versions of:
    # * Java software, i.e. JDownloader, Minecraft - A goal might be to avoid installing Java on your system.
    # * Missing software, i.e. GreenWithEnvy
    # * Proprietary software, i.e. Discord, Spotify, Steam - Hopefully the sandboxing helps security a bit!
    # * Upstream maintained, i.e. RetroArch
    ...
    "com.leinardi.gwe" # GreenWithEnvy: System utility designed to provide information, control the fans and overclock your NVIDIA card
    "org.jdownloader.JDownloader" # JDownloader: Download management tool
    "com.mojang.Minecraft" # Minecraft: Create your own world in one of the most popular video games
    "org.libretro.RetroArch" # RetroArch: Frontend for emulators, game engines and media players
    "com.spotify.Client" # Spotify: Online music streaming service
  ...
  wal = lib.importJSON ./colors.json; # Import colorscheme from json (generated by pywal)
in {
  imports = [
    ./.. # Global User Config
    (import (builtins.fetchTarball "https://github.com/rycee/home-manager/archive/master.tar.gz") { }).nixos # home-manager
  ];

  # TODO: This is creating really ugly color schemes
  ...
  home-manager = {
    ...
    users.jdf = { config, pkgs, lib, ... }: {
      accounts.email.accounts.jdf = {
        # TODO: This isn't working yet
        address = "joshuafern@protonmail.com";
        aliases = [ "joshuafern@pm.me" ];
        folders = {
          drafts = "drafts";
          inbox = "inbox";
          sent = "sent";
        ...
        passwordCommand = "echo " + secrets.hydroxide.pass;
        primary = true;
        realName = "Joshua Fern";
        userName = "JoshuaFern";
        signature = {
      ...
      home = {
        ...
        file = {
          ...
          hydroxide = {
            target = ".config/hydroxide/auth.json";
            text = secrets.hydroxide.auth;
          };
          # ~/.local/bin
          asf = {
            recursive = true;
            source = asfSource;
            target = ".local/bin/asf";
          };
          asf_execute = {
            executable = true;
            source = "${asfSource}/ArchiSteamFarm";
            target = ".local/bin/asf/ASF";
          };
          asf_config = {
            target = ".local/bin/asf/config/ASF.json";
            text = ''
              {
                "s_SteamOwnerID": "${secrets.steam.sid64}",
                "Statistics": false,
                "UpdateChannel": 0
              }
            '';
          ...
          asf_botconfig_main = {
            target = ".local/bin/asf/config/main.json";
            text = ''
              {
                "SteamLogin": "${secrets.steam.login}",
                "SteamPassword": "${secrets.steam.password}",
                "PasswordFormat": 1,
                "Enabled": true,
                "AcceptGifts": true,
          ...
          asfPlugin = {
            recursive = true;
            source = asfPluginSource;
            target = ".local/bin/asf/plugins";
          };
          asf_runner = {
            executable = true;
            target = ".local/bin/archisteamfarm";
            text = "steam-run ~/.local/bin/asf/ASF --path='/home/jdf/.local/bin/asf'";
          };
          discord_runner = {
            executable = true;
          ...
          boxtron = {
            ...
            source = builtins.fetchTarball { 
              url = "https://github.com/dreamer/boxtron/releases/download/v0.5.4/boxtron.tar.xz";
              sha256 = "1as5m2hd020b6f9k986yb7r27mmi5c59mknwsdvlmb1fp8s30ycj";
            };
            target = ".var/app/com.valvesoftware.Steam/.local/share/Steam/compatibilitytools.d/boxtron";
          };
          luxtorpeda = {
            executable = true;
            recursive = true;
            source = builtins.fetchTarball { 
              url = "https://gitlab.com/luxtorpeda/luxtorpeda/-/archive/3cec323dabd6fcdbb1266cdff853554753787b5d/luxtorpeda-3cec323dabd6fcdbb1266cdff853554753787b5d.tar.gz";
              sha256 = "0sfad2xw7dh3sr58zp5vd51iyb05xlivhmw9gwxf23ihis60y16f";
            };
            target = ".var/app/com.valvesoftware.Steam/.local/share/Steam/compatibilitytools.d/luxtorpeda";
          };
          proton_ge = {
            executable = true;
            recursive = true;
            source = builtins.fetchTarball { 
              url = "https://github.com/GloriousEggroll/proton-ge-custom/releases/download/5.8-GE-2-MF/Proton-5.8-GE-2-MF.tar.gz";
              sha256 = "12pk1bvjrziszglbrc6f0i555b19ycmf3cc70k63d53lyz3ra9vp";
            };
            target = ".var/app/com.valvesoftware.Steam/.local/share/Steam/compatibilitytools.d/proton-ge";
          };
          roberta = {
            executable = true;
            recursive = true;
            source = builtins.fetchTarball { 
              url = "https://github.com/dreamer/roberta/releases/download/v0.1.0/roberta.tar.xz";
              sha256 = "1kr97x9nr3nsvr3imf844khajijfm5mi7364pcrr2almfzyn1lga";
            };
            target = ".var/app/com.valvesoftware.Steam/.local/share/Steam/compatibilitytools.d/roberta";
          };
        };
        packages = with pkgs; [
          # applications/audio
          ...
          # NUR
          nur.repos.joshuafern.checkra1n # Jailbreak for iPhone 5s though iPhone X, iOS 12.3 and up
      ...
      xsession = {
        ...
        initExtra = ''
          # Services
          ${pkgs.hydroxide}/bin/hydroxide serve &
          # Setup
